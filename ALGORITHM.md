...
손실 거래 분석 (`extract_loss_trades.py` 실행 결과):
- 손실 건수: 74건
- 총 손실금액: -4,387,012원
- 평균 손익률: -9.01%
- 최대 손실률: -42.51%
- **전체 100% stop_loss (고정 손절)** — Engine A/B 조기청산 미발동 or 미기록

### 개선 검토 포인트
1. stop_loss 기준 10% → 손절 전 Engine 청산 조건이 충분히 먼저 작동하는지 확인
2. 매수 시 tech_score 분포: 74건의 매수점수 분포로 최적 min_score 재산정
3. 보유일수: 단기 손절(1~3일)이 많은지, 장기 보유 후 손절인지 패턴 분석
4. 엔진별 손실률: Engine A vs B 비교 필요
